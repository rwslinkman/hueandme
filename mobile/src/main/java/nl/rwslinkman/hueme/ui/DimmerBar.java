...
public class DimmerBar extends SVBar
{
    public static final String TAG = DimmerBar.class.getSimpleName();
    /*
    ...
    private OnDimmerValueSelectedListener mOnDimmerValueSelectedListener;
    private int oldDimmerListenerColor;

    /**
     * Used to inform listener of DimmerBar color changes
     */
    public interface OnDimmerValueSelectedListener
    {
        // The currently selected color will the given
        void onDimmerValueSelected(int dimmerValue);
    }

    public DimmerBar(Context context) {
        super(context);
        init(null, 0);
    }

    public DimmerBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public DimmerBar(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs, defStyle);
    }

    public void setOnDimmerValueSelectedListener(OnDimmerValueSelectedListener listener)
    {
        this.mOnDimmerValueSelectedListener = listener;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mIsMovingPointer) {
                    this.performMoveAction(dimen);
                }
                break;
            case MotionEvent.ACTION_UP:
                mIsMovingPointer = false;
                // At this point, the user releases the bar
                if(this.mOnDimmerValueSelectedListener != null && this.mColor!= this.oldDimmerListenerColor)
                {
                    // If needed, inform listener
                    this.mOnDimmerValueSelectedListener.onDimmerValueSelected(this.mColor);
                    this.oldDimmerListenerColor = this.mColor;
                }
                break;
        }
        return true;
    ...
    private void performMoveAction(float dimen)
    {
